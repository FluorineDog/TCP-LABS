...
static void bbr_set_tso_segs_goal(struct sock *sk) {
  ...
  bbr->tso_segs_goal =
      min(tcp_tso_autosize(sk, tp->mss_cache, min_segs), 0x7FU);
}
...
static bool bbr_is_next_cycle_phase(struct sock *sk,
  ...
  struct tcp_sock *tp = tcp_sk(sk);
  struct bbr *bbr = inet_csk_ca(sk);
  bool is_full_length = tcp_stamp_us_delta(tp->delivered_mstamp,
                                           bbr->cycle_mstamp) > bbr->min_rtt_us;
  u32 inflight, bw;
 ...
 */
static u32 bbr_undo_cwnd(struct sock *sk) { return tcp_sk(sk)->snd_cwnd; }
